...
int readSMSNum() {  //read in the number of text messages on the buffer and send back
  ...
  bool lookforFirstComma = false;
  bool lookforSecondComma = false;
  sprintf(response, "%s", "");
  sprintf(numIn, "%s", "");
  flushFonaSerial();
  ...
  while (fonaSerial->available())
  {
    ...
    for (int x = 0; x < fonaSerial->available() ; ++x)
    {
      ...
      if (writeResponse)
      {
        if (a == 'M' && b == 'S')
        {
          lookforFirstComma = true;
          Serial.println("Found SM");
        }
        if (lookforSecondComma)
        {
          if (a == ',')
          {
            secondCommaLocation = x;
            lookforSecondComma = false;
            writeResponse = false;
            Serial.print("Found second Comma at "); Serial.println(x);
          }
        }
        if (lookforFirstComma)
        {
          if (a == ',')
          {
            firstCommaLocation = x;
            lookforFirstComma = false;
            lookforSecondComma = true;
            Serial.print("Found first Comma at "); Serial.println(x);
          }
        }
      }
  ...
  for (int x=firstCommaLocation+1 ; x<secondCommaLocation ;++x)
  {
    sprintf(numIn,"%s%c",numIn,response[x]);
  }
  
  Serial.print("Response from Fona -"); Serial.println(response);
  //Serial.print("Number In -"); Serial.println(numIn);
  smsnum = atoi(numIn);
  //Serial.print("to an int ");Serial.println(smsnum);
  Serial.print("You have "); Serial.print(smsnum); Serial.println( "messages");
  sprintf(response, "%s", "");
  Serial.println("Response should be empty");
...
void sendSampleReply(char*from)
{
  ...
  Serial.print(month(sampled)); Serial.print("/"); Serial.print(day(sampled)); Serial.print("/"); Serial.print(year(sampled)); Serial.print("  "); Serial.print(hour(sampled)); Serial.print(":"); Serial.print(minute(sampled)); Serial.print(":"); Serial.println(second(sampled));
  char message[100];
  int y = sprintf(message, "%s%i%s%i%s%i%s%i%s%i%s%i%s%i%s", "Sampled bottle ", getBottleNumber(), " @ ", month(sampled), "/", day(sampled), "/", year(sampled), "   ", hour(sampled), ":", minute(sampled), ":", second(sampled), "EDT");
...
void saveData() {
  ...
  dtostrf(conductivity, 3, 4, Conductivity1Char);
  //dtostrf(conductivity2, 3, 4, Conductivity2Char);
  //dtostrf(conductivity3, 3, 4, Conductivity3Char);
  dtostrf(permittivity, 3, 4, Permittivity1Char);
  //dtostrf(permittivity2, 3, 4, Permittivity2Char);
  //dtostrf(permittivity3, 3, 4, Permittivity3Char);
  ...
  } else
  {
    ...
    if (writeNum == 0)
    {
      ...
      myFile.println("TIMESTAMP(EST),temp1 (degc),moisture1 (%),permittivity,Battery Voltage (V)");
    }
    delay(10);
    buildDateTime();
