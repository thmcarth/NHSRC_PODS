...
char GPS[] = "32.881050 ,-78.871917 0"; //needs a (space 0) at the end, example ="35.068471,-89.944730 0"
int unit = 1;
...
void setup() {
  Serial.begin(9600);
  int countdownMS = Watchdog.enable(120000);
...
void checkTexts()
{
  ...
  if (numSMS > 0)
  {
    ...
    if (commandNum == 0)
    {
      sendSMS(senderNum, "Did not understand the command.\n 1= Rail Voltages \n 2=Sample ISCO \n 3=Hydra1 \n 4=Hydra2");
      sprintf(Message, "%s", "");
      sprintf(Message, "%s", "\n 5=Hydra3 \n 6=Cheap Sensor \n 7=Turn");
      if (HPRail)
      {
...
bool getWaterLevel()
{
  int reading = analogRead(levelPin);
  levelReading = reading;
  Serial.print("Level pin = "); Serial.println(reading);
  if (reading < 320)
  {
    return true;
  } else
