...
void checkTexts() //reads SMS(s) off the Fona buffer to check for commands
{
  ...
  if (numSMS > 0)
  {
    ...
    {
      commandNum = -1;
      if (getWaterLevel()) //Only if there's water in the pan..
      {
      ...
      {
        sendSMS(senderNum, "Water level too low. Cannot sample"); //If it fails, because water level is low, let the user know
      }
    }
