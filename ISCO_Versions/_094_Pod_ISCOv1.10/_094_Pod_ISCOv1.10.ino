...
//Version 1.11 - Added ISCO rain bucket code. - THM
...
int unit = 1;
char versionNum[] = "_094_Pod_ISCOv1.10";
...
void loop() {
  ...
  int cr = getCurrentRain();
  Serial.print("Current Rain is " );Serial.println(cr);
  int dr = getDaysRain();
  Serial.print("Today's Rain is " );Serial.println(dr);
...
void saveData() {
  ...
  } else
  {
    ...
    if (writeNum == 0)
    {
      ...
      myFile.println("Probe2_temp, Probe2_moisture, Probe2_conductivity, Probe2_permitivity,Probe3_temp, Probe3_moisture, Probe3_conductivity, Probe3_permitivity,Davis_tips,Davis_inch_tips");
    }
...
void I2C_rain() {
  ...
  {
    inch_tips = tips / 100;
    //tips = tips / Intensity_period; //Just leave it as tips
...
int getCurrentRain()
{
  ...
  int index = getIndex(data, 'R', 1, str_len);
  ...
  } else
  {
    ...
    if (data[index + 1] == 'A')
    {
      ...
      char number[3] = {data[index + 2], data[index + 3], data[index + 4]};
...
int getDaysRain()
{
  ...
  char data[str_len];
  inputData.toCharArray(data, str_len);
  int index = getIndex(data, 'C', 1, str_len);
...
int getIndex(char data[], char delimiter, int num, int str_len)
{
  ...
  } while (commaNum < num && strIndex < str_len);
