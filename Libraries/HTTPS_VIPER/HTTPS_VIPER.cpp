...
if (dataidx == 0)
{
sprintf(post_Data,"P%s%s;%i;%s;Green",post_Data,name,value,units);
}else
{
...
if (dataidx == 0)
{
sprintf(post_Data,"P%s%s;%f;%s;Green",post_Data,name,value,units);
}else
{
...
if (dataidx == 0)
{
sprintf(post_Data,"P%s%s;%s;%s;Green",post_Data,name,value,units);
}else
{
...
void HTTPS_VIPER::setup_APN(){
	s->println("AT+CGSOCKCONT=1,IP,wholesale,0.0.0.0,0,0");
	delay(10);
	s->println("AT+CSOCKSETPN=1");
}
